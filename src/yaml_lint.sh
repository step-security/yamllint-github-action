...
function yaml_lint {
    ...
    set -- yamllint
    [ -n "${yamllint_strict}" ] && set -- "$@" "${yamllint_strict}"
    [ -n "${yamllint_config_filepath}" ] && set -- "$@" --config-file "${yamllint_config_filepath}"
    [ -n "${yamllint_config_datapath}" ] && set -- "$@" --config-data "${yamllint_config_datapath}"
    [ -n "${yamllint_format}" ] && set -- "$@" --format "${yamllint_format}"
    set -- "$@" "${yamllint_file_or_dir}"
    "$@" > lint_result.txt
    lint_exit_code=${?}
...
*Workflow: \`${GITHUB_WORKFLOW}\`, Action: \`${GITHUB_ACTION}\`, Lint: \`${yamllint_file_or_dir}\`*"
        ...
        if [ -n "${lint_comment_url}" ] && [ "${lint_comment_url}" != "null" ] && [ -n "${GITHUB_ACCESS_TOKEN}" ]; then
            ...
            echo "lint: warning: skipping PR comment (missing comment URL or access token)"
        fi
    fi
